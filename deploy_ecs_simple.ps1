...
Write-Host "2️⃣  Obteniendo VPC y subnets..." -ForegroundColor Yellow
...
Write-Host "`n3️⃣  Creando security group..." -ForegroundColor Yellow
...
# Crear log group
Write-Host "Creando CloudWatch log group..." -ForegroundColor Cyan
...
Write-Host "5️⃣  Registrando en ECS..." -ForegroundColor Yellow
...
# Crear servicio
Write-Host "6️⃣  Creando ECS service..." -ForegroundColor Yellow
$service_exists = aws ecs list-services --cluster $ECS_CLUSTER --region $AWS_REGION 2>&1 | ConvertFrom-Json
...
Write-Host "7️⃣  Esperando a que las tareas se estabilicen..." -ForegroundColor Yellow
...
# Obtener IP pública
Write-Host "`n8️⃣  Obteniendo dirección IP pública..." -ForegroundColor Yellow
